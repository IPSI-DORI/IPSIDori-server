...
public class AuthController {
	...
	public ResponseEntity<ApiResponse<TokenDto>> kakaoLoginCallback(
		@Parameter(hidden = true) @AuthenticationPrincipal OAuth2User oauth2User
	) {
		if (oauth2User == null) {
			throw AuthNotFoundException.oauthUserNotFound();
		}
		return ApiResponse.ok(queryAuthService.oauth2Login(oauth2User));
	}
	...
	public ResponseEntity<ApiResponse<TokenDto>> reissue(
		...
		return ApiResponse.ok(queryAuthService.reissue(requestDto.refreshToken()));
	}
}
