...
public class AuthController {
	...
	public ResponseEntity<CustomApiReponse<TokenDto>> kakaoLoginCallback(
		@Parameter(hidden = true) @AuthenticationPrincipal OAuth2User oauth2User
	) {
		if (oauth2User == null) {
			throw AuthNotFoundException.oauthUserNotFound();
		}
		return CustomApiReponse.ok(queryAuthService.oauth2Login(oauth2User));
	}
	...
	@PostMapping("/reissue")
	public ResponseEntity<CustomApiReponse<TokenDto>> reissue(
		@Valid @RequestBody TokenReissueRequest requestDto
	) {
		return CustomApiReponse.ok(queryAuthService.reissue(requestDto.refreshToken()));
	}
}
